...
- **Delete a customer** — enabled only when a customer is selected (command `CanExecute` gating).
- **Toggle navigation side** — moves the navigation panel between the left and right of the window.
...
- **Data access behind an interface** — `ICustomerDataProvider` decouples the UI from the data source, so the mock provider could be swapped for a real database or API without touching the ViewModels.
...
- **Value converters** — UI layout (navigation side) is driven by bound state through an `IValueConverter`.
...
```bash
git clone https://github.com/Or-Arbiv1/WiredBrainCoffee.CustomersApp.git
...
1. Open `WiredBrainCoffee.CustomersApp.sln`.
2. Set `WiredBrainCoffee.CustomersApp` as the startup project.
3. Press **F5**.

The app starts with two sample customers supplied by the mock data provider.
